...
#
# Это тот самый контейнер, где Chromium открывает страницу, адрес которой
# назвал произвольный посетитель. В окружении рядом лежат ключ эквайринга,
# пароль почтового ящика, токен бота и секрет подписи сессий. Root здесь не
# нужен ни для чего, а цена его наличия — вся связка разом при одной дыре
# в браузере.
