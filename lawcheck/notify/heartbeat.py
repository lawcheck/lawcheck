...
JOBS: dict[str, timedelta] = {
    ...
    "email-jobs": timedelta(hours=26),       # контейнер followups, ежедневно 06 UTC
}
# Отметки сюда шлют хостовые cron-скрипты (через HTTP /internal/heartbeat) и
# контейнер followups (через tools/mark_email_jobs прямо в БД — INTERNAL_KEY
# рассыльщику намеренно не выдан). Помечается только прогон, где все три
# рассылки вышли с кодом 0: `|| true` в цикле контейнера глушит ошибки, и без
# отметки сторож — единственный, кто заметит смерть SMTP.
