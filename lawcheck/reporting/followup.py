...
def render(ctx: dict) -> tuple[str, str, str]:
    ...
    text_lines += [
        "",
        # Лестница Pro → Business → персональное сопровождение. Состав тарифов
        # держать в согласии с pricing.html, обещание про подпись юриста —
        # с п. 2.1 оферты. Сопровождение — единственная позиция без прайса:
        ...
        "устраняю все нарушения и остаюсь на связи по вопросам от регулятора.",
        "Формат и цену подбираем под задачу — ответьте на это письмо,",
        f"и я предложу вариант под {site}.",
        "",
    ...
    parts += [
        ...
        "разбираю, устраняю все нарушения и остаюсь на связи по вопросам "
        "от регулятора. Формат и цену подбираем под задачу — ответьте "
        f"на это письмо, и я предложу вариант под {e(site)}.</p></div>",
